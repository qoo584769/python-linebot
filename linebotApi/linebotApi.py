		...
		if image_response.status_code == 200:
			...
			with open('rich_menu_image.png', 'rb') as f:
				...
				line_bot_blob_api.set_rich_menu_image(
					rich_menu_id, body=image_bytes, _headers={'Content-Type': 'image/png'}
				)
			# 設定為預設快速選單
...
def text_match(text):
	...
	find_email_text = (
		role_email.findall(find_logon_text[1])
		if len(find_logon_text) > 1
		else find_logon_text[0]
	)
	...
	match text:
		...
		case text if text.startswith('註冊') and User.exists(email):
			return '帳號已存在'
	...
	if msg_type == 'text':
		if text == '雷達回波圖' or text == '雷達回波':
			messaging_api.push_message(
				PushMessageRequest(to=user_id, messages=[TextMessage(text='取得資料中....')])
			)
			...
			reply_request = ReplyMessageRequest(
				reply_token=event.reply_token, messages=[img_message]
			)
		...
		if text == '電影':
			...
			messaging_api.push_message(
				PushMessageRequest(
					to=user_id, messages=[TextMessage(text='正在啟動電影API....')]
				)
			)
